...
runtime.onInstalled.addListener(function () {
    ...
    proxy.settings.get({
        ...
        // runtime.sendMessage({ "current_mode": current_mode }, (response) => {
...
function setMode(curr_mode) {
    if (curr_mode === MODE_SYSTEM) {
        proxy.settings.set({ value: config_SOCKS_5, scope: 'regular' }, () => {
        ...
        proxy.settings.set({ value: config_default, scope: 'regular' }, () => {
